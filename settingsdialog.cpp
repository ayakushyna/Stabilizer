    ...
    m_intValidator(new QIntValidator(0, 4000000, this))
{
    ...
    setWindowTitle("Настрокйки подключения");
}
...
void SettingsDialog::showPortInfo(int idx)
{
    ...
    m_ui->manufacturerLabel->setText(tr("Производитель: %1").arg(list.count() > 2 ? list.at(2) : tr(blankString)));
    m_ui->serialNumberLabel->setText(tr("Серийный номер: %1").arg(list.count() > 3 ? list.at(3) : tr(blankString)));
    m_ui->locationLabel->setText(tr("Локация: %1").arg(list.count() > 4 ? list.at(4) : tr(blankString)));
    m_ui->vidLabel->setText(tr("Идентификатор поставщика: %1").arg(list.count() > 5 ? list.at(5) : tr(blankString)));
    m_ui->pidLabel->setText(tr("Идентификатор продукта: %1").arg(list.count() > 6 ? list.at(6) : tr(blankString)));
}
